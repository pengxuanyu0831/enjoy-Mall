...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection arg0) {
        ...
        // 添加用户权限信息
        List<MenuModel> permissionList = iLoginService.queryPermissionList(JSONObject.toJSONString(user));
...
//        SecurityUtils.getSubject().getSession().setAttribute(CommonConstant.SESSION_USER_PERMISSIONS,info.getStringPermissions());
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        try{
            ...
            LinkedHashMap userMap = (LinkedHashMap)user.get("user");
            SysUser sysUser = JSONObject.parseObject(JSONObject.toJSONString(userMap), SysUser.class);
            user.remove("user");
